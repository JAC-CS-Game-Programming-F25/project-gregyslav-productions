...
export default class Hitbox {
	...
	didCollide(target) {
		let temp = null
		for (let corner in target.corners) {
			temp = new Vector(target.corners[corner].x + target.position.x, target.corners[corner].y + target.position.y)
			if (this.checkCollisionOnPoint(temp)) {
				return true;
			}
		};

		for (let corner in this.corners) {
			temp = new Vector(this.corners[corner].x + this.position.x, this.corners[corner].y + this.position.y)
			if (target.checkCollisionOnPoint(temp)) {
				return true;
			}
		}
